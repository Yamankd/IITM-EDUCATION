...
const GalleryView = () => {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (files.length === 0)
      return showError("Please select at least one image");
  ...
  const handleBulkDelete = async () => {
    ...
    const isConfirmed = await showConfirm({
      title: "Delete Images",
      message: `Are you sure you want to delete ${selectedImages.length} images?`,
      type: "danger",
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      showError("Failed to update image details");
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-center bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 gap-4 transition-colors">
        <div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100 dark:border-gray-700 transition-colors">
